...
class RoomView(generics.ListAPIView):
    """
    API view to retrieve a list of all rooms.
    """
...
class GetRoom(APIView):
    ...
    def get(self, request, format=None):
        """
        Handle GET request to retrieve room details by code.
        """
...
class JoinRoom(APIView):
    """
    API view to join a room by its code.
    """
    lookup_url_kwarg = 'code'
...
class UserInRoom(APIView):
    ...
    def get(self, request, format=None):
        """
        Handle GET request to check if a user is in a room.
        """
        if not self.request.session.exists(self.request.session.session_key):
            self.request.session.create()
...
class UpdateRoom(APIView):
    """
    API view to update room settings.
    """
    serializer_class = UpdateRoomSerializer

    def patch(self, request, format=None):
        """
        Handle PATCH request to update room settings.
        """
        if not self.request.session.exists(self.request.session.session_key):
            self.request.session.create()
